...
- для самовывоза должен быть указан `pickup_point_id`;
- для доставки на адрес должны быть указаны город, улица и дом;
- выбранный пункт выдачи должен существовать и быть активным.

#### PaymentService

Отвечает за бизнес-процесс оплаты:

- для оплаты картой дополнительные данные не требуются;
- для оплаты в кредит обязательны кредитный провайдер и срок кредита;
- срок кредита должен быть допустимым значением.
...
## 4. Базовая логика оформления заказа
...
1. Получить и сделать валидацию входных данных.
