...
Данная сущность добавлена для соответствия принципу SRP, упрощения расширения, хранения общих полей доставки при необходимости.
...
Также для упрощения не контролируем доступные остатки товаров (для тестового это избыточно).
...
- создание указанных выше сущностей и их заполнение тестовыми данными;
- API и бизнес-логика (с разделением по указанным выше зонам ответственности); 
- Тесты;
- документация по API;
- проверка на соответствие PSR-12;
- при необходимости разработка frontend-части (например, Blade или Inertia);
- общая документация (readme).
